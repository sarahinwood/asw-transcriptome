...
##231 genes, with 497 transcripts have prokaryotic annots
length(unique(prokaryotic$`#gene_id`))

############
# bacteria #
############
#94 genes, 167 transcripts, from 53 different species
...
# viruses #
###########
#115 genes, 245 transcripts, from 41 different species
viruses <- dplyr::filter(split.first.blastx, grepl('Viruses', V7))
length(unique(viruses$`#gene_id`))
length(unique(viruses$V7))

viruses$family <- tstrsplit(viruses$V7, "; ", keep=c(2))
genes_viral_family <- viruses[,length(unique(`#gene_id`)), by=family]
##plot
...
# archaea #
###########
#26 genes, 85 transcripts, from 7 different species
